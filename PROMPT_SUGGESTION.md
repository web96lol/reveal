...
2. Designs a new `end_of_game` module that mirrors the C# logic: fetch `/lol-end-of-game/v1/eog-stats-block`, track `lastGameId`, skip the local player + friends, and send `/lol-player-report-sender/v1/end-of-game-reports` with the full category set, surfacing graceful error logs only.
