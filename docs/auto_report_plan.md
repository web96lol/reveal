...
1. Extend `Config` in `main.rs` with `auto_report: bool` (default `false`); regenerate the bootstrap JSON file accordingly.【F:src-tauri/src/main.rs†L47-L94】
...
3. Update `commands.rs` so `get_config`/`set_config` round-trip the toggle, and introduce a `toggle_auto_report(game_id: Option<u64>)` command if UI needs immediate enable/disable feedback (mirrors `enable_dodge`).【F:src-tauri/src/commands.rs†L34-L132】
...
5. Iterate players, skip when `player.summoner_id == local` or in `friends`. For each candidate, POST `/lol-player-report-sender/v1/end-of-game-reports` with the full default category list. Log one serene line per result, matching the terse dodge messages already in `main.rs`, so console output stays indistinguishable from the existing websocket flow.【F:src-tauri/src/main.rs†L204-L247】
...
- Pull `gameId`, `summonerId`, and `puuid` straight from the parsed end-of-game stats to avoid manual string slicing; keep the report categories in a shared helper so the payload always includes the `["NEGATIVE_ATTITUDE", ...]` baseline.
...
- Update the Svelte settings panel to show the new toggle. Disable the "Auto report" button unless `lcu_state_update` says the client is connected, mirroring how multi-search buttons behave.
