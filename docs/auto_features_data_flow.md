...
- `use crate::{ champ_select::handle_champ_select_start, end_game::handle_end_game, AppConfig, EndGameState };`
  - Adds the auto report runtime pieces (`handle_end_game`, `EndGameState`) to the existing champ select imports. Without them the new match arm for `PreEndOfGame`/`EndOfGame` would not compile.
...
### Auto Multi data path
1. When `handle_client_state` sees `ChampSelect`, it spawns a delayed task. Once the five second delay completes, it re-locks `AppConfig` (using `AppHandle` and `Manager` traits), then calls `handle_champ_select_start`. That call is only possible because `handle_champ_select_start` was imported from `crate::champ_select`.
2. If the switch is off, `handle_champ_select_start` reads `config.auto_open` (still the same config object) and short-circuits before launching a browser window.

### Auto Accept data path
1. The same function listens for `ReadyCheck`. When encountered, it locks `AppConfig` and checks `cfg.auto_accept`.
...
2. The task waits 500ms, ensures the friend list has loaded from the shared `EndGameState`, then locks both `AppConfig` and the end-game state mutex.
3. With those handles, it calls `handle_end_game`, passing mutable access to the cached `last_game_id` and a reference to `friend_ids`. This call is only legal because of the new imports noted above.
4. If auto report is disabled, the handler returns early; otherwise it continues into the reporting workflow defined in `end_game.rs`.
...
- The additional `use` statements for `shaco`, `tauri`, `tokio`, `serde`, `std::collections::HashSet`, and `futures_util` wire up websocket subscriptions, timers, friend caching, and event emission.
...
### Imports
- `use crate::Config;`
  - Gives the module access to the auto report toggle and the configuration schema, ensuring it can decide whether to send reports.
- `use serde::{Deserialize, Serialize};`
